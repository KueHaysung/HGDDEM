...
export function addVoxelsToScene(scene, voxels) {
    ...
    function findPathWithStartTime(start, goal, otherPaths, currentStep, startStep) {
    ...
    function planPathsWithStartTime(objects) {
        const maxSteps = 2; // 可根据实际情况调整最大步骤数
        const paths = [];

        for (let step = 0; step < maxSteps; step++) {
            for (let i = 0; i < objects.length; i++) {
                const obj = objects[i];
                if (step >= obj.startStep) {
                    const start = obj.start;
                    const goal = obj.goal;
                    const otherPaths = paths.filter((p, index) => index!== i);
                    const path = findPathWithStartTime(start, goal, otherPaths, step, obj.startStep);
                    if (path.length > 0) {
                        paths.push({
                            path: path,
                            objectId: obj.id,
